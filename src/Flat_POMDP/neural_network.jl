...
function initialize_network(nn_params::BetaZeroNetworkParameters)
    ...
    function DenseRegularizedLayer(in_out::Pair)
        ...
        if use_batchnorm && !use_dropout
            return [
                Dense(input => output),
                BatchNorm(output, activation; momentum=batchnorm_momentum),
            ]
    ...
    if nn_params.use_cnn
        ...
        num_filters_value = 1
        out_conv_size = prod([
            input_size[1] - 2 * (filter[1] - 1),
            input_size[2] - 2 * (filter[2] - 1),
            num_filters2,
        ])
        out_conv_size_policy = prod([
            input_size[1] - 2 * (filter[1] - 1) - (filter_policy[1] - 1),
            input_size[2] - 2 * (filter[2] - 1),
            num_filters_policy,
        ])
        out_conv_size_value = prod([
            input_size[1] - 2 * (filter[1] - 1) - (filter_value[1] - 1),
            input_size[2] - 2 * (filter[2] - 1),
            num_filters_value,
        ])
        ...
        if nn_params.use_deepmind_arch
            ...
            return Chain(
                ...
                Parallel(
                    vcat;
                    value_head=Chain(
                        ConvRegularizedLayer(
                            filter_value, num_filters2 => num_filters_value
                        )...,
                        Flux.flatten,
                    ...
                    policy_head=Chain(
                        ConvRegularizedLayer(
                            filter_policy, num_filters2 => num_filters_policy
                        )...,
                        Flux.flatten,
        ...
        return Chain(
            ...
            Parallel(
                ...
                policy_head=Chain(
                    DenseRegularizedLayer(ℓs => ℓs)..., Dense(ℓs => action_size), softmax
                ),
            ),
        )
    end
...
function calc_loss_weight(nn_params::BetaZeroNetworkParameters)
    return calc_loss_weight(nn_params.action_size)
end
...
function train(
    ...
    data_train_set = sample_data(
        solver.data_buffer_train, n_train; sample_more_than_collected
    ) # sample from last `n_buffer` simulations.
    data_valid_set = sample_data(
        solver.data_buffer_valid, n_valid; sample_more_than_collected
    ) # sample from last `n_buffer` simulations.
    ...
    if n_train < nn_params.batchsize
        batchsize = n_train
        @warn(
            "Number of observations less than batch-size, decreasing the batch-size to $batchsize",
            maxlog = 1
        )
    else
    ...
    function loss(x, y; w=value_loss_weight, info=Dict())
        local ỹ = f(x)
        n = size(ỹ, 1) - 1
        ...
        return value_loss + policy_loss + regularization
    end
    ...
    function logging_fn(
        ...
        return string(
            ...
            epoch,
            "\t Loss Train: ",
            round(loss_train; digits),
            " [",
            round(loss_train_value; digits),
            ", ",
            round(loss_train_policy; digits),
            "]\t Loss Val: ",
            round(loss_valid; digits),
            " [",
            round(loss_valid_value; digits),
            ", ",
            round(loss_valid_policy; digits),
            "]\t|\t Sign Acc. Train: ",
            rpad(round(acc_train; digits), digits + 2, '0'),
            "\t Sign Acc. Val: ",
            rpad(round(acc_valid; digits), digits + 2, '0'),
            extra,
        )
    end
    ...
    @conditional_time verbose for e in 1:training_epochs
        ...
        if e % nn_params.checkpoint_frequency == 0
            checkpoint_condition = if nn_params.checkpoint_validation_loss
                loss_valid < checkpoint_loss_valid
            else
                loss_train < checkpoint_loss_train
            end
    ...
    if nn_params.use_checkpoint
        ...
        verbose && println(
            logging_fn(
                checkpoint_epoch,
                checkpoint_loss_train,
                checkpoint_loss_train_value,
                checkpoint_loss_train_policy,
                checkpoint_loss_valid,
                checkpoint_loss_valid_value,
                checkpoint_loss_valid_policy,
                checkpoint_acc_train,
                checkpoint_acc_valid;
                extra=" [Final network checkpoint]",
            ),
        )
        f = f_checkpoint
    end
    ...
    if nn_params.verbose_plot_frequency != Inf
        ...
        try
            ...
            value_model = if normalize_input
                f(unnormalize_x(cpu(x_valid)))[1, :]
            else
                f(cpu(x_valid))[1, :]
            end
            value_data =
                normalize_output ? unnormalize_y(cpu(y_valid))[1, :] : cpu(y_valid)[1, :]

            value_distribution = Plots.histogram(
                value_model; alpha=0.5, label="model", c=:gray, title="values: $key"
            )
            Plots.histogram!(value_data; alpha=0.5, label="data", c=:navy)
            nn_params.save_plots &&
                Plots.savefig(nn_params.plot_value_distribution_filename)
            ...
            value_model_training = if normalize_input
                f(unnormalize_x(cpu(x_train)))[1, :]
            else
                f(cpu(x_train))[1, :]
            end
            value_data_training =
                normalize_output ? unnormalize_y(cpu(y_train))[1, :] : cpu(y_train)[1, :]
...
function next_action(
    problem::Union{BeliefMDP,POMDP},
    belief,
    f::Union{Chain,EnsembleNetwork},
...
"""
function tune_network_parameters(
    pomdp::POMDP,
    solver::BetaZeroSolver;
            ...
            for λ in λs
                for lr in learning_rates
                    ...
                    generate_data!(
                        ...
                        solver,
                        f_prev;
                        use_random_policy=solver.use_random_policy_data_gen,
                        inner_iter=solver.n_data_gen,
                        outer_iter=1,
                    )
                    f_curr = train(
                        deepcopy(f_prev), solver; verbose=solver.verbose, results=results
                    )
