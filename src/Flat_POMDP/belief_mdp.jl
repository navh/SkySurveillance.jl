...
Implementation from: https://github.com/josh0tt/TO_AIPPMS/blob/main/SBO_AIPPMS/GP_BMDP_Rover/belief_mdp.jl
Which is based on: POMDPs.jl's GenerativeBeliefMDP 
"""
...
function BeliefMDP(pomdp::P, up::U, belief_reward) where {P<:POMDP,U<:Updater}
    ...
    return BeliefMDP{P,U,typeof(b0),actiontype(pomdp)}(pomdp, up, belief_reward)
end

function POMDPs.gen(bmdp::BeliefMDP, b, a, rng::AbstractRNG)
    s = rand(rng, bmdp.pomdp, b)
    if isterminal(bmdp.pomdp, s)
        bp = bmdp_handle_terminal(
            bmdp.pomdp, bmdp.updater, b, s, a, rng::AbstractRNG
        )::typeof(b)
        return (sp=bp, r=0.0)
    end
...
POMDPs.isterminal(bmdp::BeliefMDP, b::FlatBelief) = isterminal(bmdp.pomdp, b)
