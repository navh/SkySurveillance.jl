...
DWELL_TIME_SECONDS = 40e-3  # ∈ [10ms,40ms] # from Jack
...
function POMDPTools.render(pomdp::FlatPOMDP, step)
    ...
    # Add targets
    plot!(plt, [(target.x, target.y) for target in step.s.targets]; seriestype=:scatter)
    ...
    plot!(plt, beam; fillcolor=:red, fillalpha=1)

    # Plot observations
    # plot!(plt, [(target.x, target.y) for target in step.o]; seriestype=:scatter)
